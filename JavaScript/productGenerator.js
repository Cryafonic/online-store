...
detailedProduct = JSON.parse(sessionStorage.getItem("imageCollector"));

//Generates the individual products on the page.
$(document).ready(function () {
    for (let i in jsonData.products) {
        if (detailedProduct[0] === jsonData.products[i].image) {
            let outSideDiv = $(`<div class="card bg-dark text-white"></div>`);
            outSideDiv.append(`<img src="${jsonData.products[i].image}" class="card-img" alt="${jsonData.products[i].name}" style="max-height: 100%;max-width: 40%"/>`);
            let inSideDiv = $(`<div class="card-img-overlay"></div>`)
            inSideDiv.append(`<h5 class="card-title">${jsonData.products[i].name}</h5><p class="card-text">${jsonData.products[i].description}</p><p class="card-text">R${jsonData.products[i].price}</p><button class="btn btn-outline-success quickAddCart" onclick="addToCart(), totalPriceAlert()">Add to cart</button>`);
            $(outSideDiv).append(inSideDiv);
            $("#productDetails").append(outSideDiv);
        };
    };
});

// Adds products to the session key.
function addToCart() {
...
// Adds the totals price to the products.
